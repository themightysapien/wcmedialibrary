<template>
  <div>
    <input type="file" ref="auto_file_input" style="display: none" @change="fileChanged" :multiple="multiple" />

    <span :class="inputContainerClass" @click="$refs['auto_file_input'].click()">
...
export default {
  ...
  props: {
    ...
    resetFilesOnUpload: { default: false },
  ...
  mounted() {
    ...
    this.fileList = this.files.filter((item) => item);
  },
  computed: {
    isBtnVisible() {
      if (
